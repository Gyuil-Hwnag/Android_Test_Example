...
class CalcViewModelTest {
    ...
    fun setUp() {
        ...
        Mockito.`when`(calculations.calculateArea(2.1)).thenReturn(2.1*2.1*Math.PI)
        Mockito.`when`(calculations.calculateCircumference(1.0)).thenReturn(1.0*2*Math.PI)
    ...
    fun calculateArea_radiusSent_updateLiveData(){
        ...
        Truth.assertThat(result).isEqualTo((2.1*2.1*Math.PI).toString())
    }
    ...
    fun calculateCircumference_radiusSent_updateLiveData(){
        ...
        Truth.assertThat(result).isEqualTo((1.0*2*Math.PI).toString())
    }
}